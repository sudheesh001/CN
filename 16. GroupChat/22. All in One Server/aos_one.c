...
#define SERVING_PATH "/tmp/serving_path_one"
...
void serv_client(void *arg){
	...
	int n =  write(newsockfd[my_id],"you are connected to service server",36);
	if( n < 0)
		...
		while(1){
			bzero(buf, 100);
			n = recv(newsockfd[my_id], buf, sizeof(buf), 0);
			...
			n =  send(newsockfd[my_id],"got msg",8,0);
			if( n < 0)
				perror("failed to write back to client");
		}
